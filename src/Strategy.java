...
class Option implements Comparable<Option> {
    int angle, player_id, annoys, distance,angle_diff;
    int score;
    Cor des;
...
class Strategy {
    ...
    static Player[] init_players() {
        ...
        players[1] = new Player("E. Hajisafi", new Position(-4, 2));
        players[2] = new Player("M. Karimi", new Position(-4, -2));
        players[3] = new Player("M. Navidkia", new Position(-1, 1));
        players[4] = new Player("H. Aghili", new Position(-1, -1));
    ...
    private static boolean is_mohafez(Cor Player_Cor ,Cor Ball_Cor) {
...
//        System.out.println("annoys:"+annoys+" , id:"+ Player_id +" , Ax:"+A.x+", y:"+A.y +" , Bx:"+B.x+", y:"+B.y);
    ...
    private static int get_Score(Cor Player_Cor, Cor Ball_Cor, Cor Gate_Cor, int angle, int i, int Player_id, Game game, Option option) {
        int score = 0;
        if (is_mohafez(Player_Cor , Ball_Cor)) {
            score -= 7;
        }
        if (i == 0) {
...
//            System.out.println("O_o: " + Gate_Cor.y);
            int tafazol_angle = Math.abs(angle - find_angle(Ball_Cor, Gate_Cor));
            if (tafazol_angle > 180)
                tafazol_angle = 360 - tafazol_angle;
            score += 12 - (tafazol_angle / 9);
            ...
            score -= 2*(int) Math.floor(player_ball_distance);
            ...
            score -= 2*(int) Math.floor(player_shadow_distance);
        ...
        for (int j = 1; j < 24; j++) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (i == 0) {
                    ...
                    option.score += Math.abs((int) ((0.3)*option.score));
                    option.des = Gate_Cor;
    ...
    public static Triple do_turn(Game game) {
        ...
        for (Option option : priorityQueue) {
            System.out.println(option.player_id + ",score:" + option.score + ",angle:" + option.angle + ",distance:"
                    + option.distance + ",annoys:" + option.annoys+",des cor.x:"+option.des.x+",des.cor.y:"+option.des.y+",angle_diff:"+option.angle_diff);
        }
        ...
        if (My_Score + 5 < Opp_Score && myturns % Check_cycle == 0 &&  priorityQueue.size()>=1) {
