...
class Strategy {
    private static int goal_period = 0;
    private static int Opp_Scores = 0;
    private static Random rnd = new Random();
    private static List<List<Triple>> movements = new ArrayList<>();
    private static List<Triple> moves = new ArrayList<>();
    ...
    private static int get_Score(Cor Player_Cor, Cor Ball_Cor, Cor Gate_Cor, int i, int Player_id, Game game, Option option) {
        ...
        if (i == 0) {
            ...
            option.annoys = (find_annoy(Player_Cor, player_shadow, false, Player_id, game) +
                    ...
                    find_annoy(Ball_Cor, Gate_Cor, true, Player_id, game));
            int tafazol_angle = Math.abs(find_angle(player_shadow, find_tok(Ball_Cor,Gate_Cor,true)) - find_angle(Ball_Cor, Gate_Cor));
            if (tafazol_angle > 180)
                tafazol_angle = 360 - tafazol_angle;
            option.angle_diff = tafazol_angle;
            score += 10 - (tafazol_angle / 9);
...
//        if (score < 4)
    ...
    static Triple do_turn(Game game) {
        ...
        if (Opp_Score != 0){
            if (Opp_Scores != Opp_Score){
                // means that Opp score has changed and Opp has goal
                movements.add(moves);
                moves = Collections.unmodifiableList(moves);
                moves = new ArrayList<>();
                Opp_Scores = Opp_Score;
            }else{
                if (movements.size() != 0 && movements.get(movements.size()-1).size() == moves.size() + 1){
                    int i = 0;
                    for (; i < moves.size(); i++) {
                        if (moves.get(i) != movements.get(movements.size()-1)){
                            break;
                        }
                    }
                    if (i == moves.size()){
        ...
        if (best_option == null) {
            ...
            if (danger){
                option = optionPriorityQueue.poll();
            }
